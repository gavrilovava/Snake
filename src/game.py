...
from src.level import level, start_choise
...
def gameLoop():
    ...
    while not constant.game_over:
        ...
        for i in range(parameter.dis_width):
            pygame.draw.rect(parameter.dis, constant.colour.red, [i, constant.const.food_one, constant.const.food_x, constant.const.food_y])
        ...
        if game_param.x1 >= parameter.dis_width or game_param.x1 < 0 or game_param.y1 >= parameter.dis_height or game_param.y1 < constant.const.edge:
